...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-sm-12">
			<div class="row">
				<form method="post" action="{{url('updatedesa')}}/{{$data->did}}" enctype="multipart/form-data">
					{{ csrf_field() }}
					...
					<div class="col-sm-3">
						...
						<div class="content cat-content">
							...
							<p><label for="{{$cat->cinid}}"><input type="checkbox" name="category_id[]" value="{{$cat->cinid}}" @if(in_array($cat->cinid,$postcat)) checked @endif> {{$cat->title}}</label></p>
							@endforeach
						</div>
						<div class="content featured-image">
							<h4>Featured Image <span class="pull-right"><i class="fa fa-chevron-down"></i></span></h4><hr>
							@if($data->image != '')
							<p><img id="output" style="max-width: 100%" src="{{url('desas')}}/{{$data->image}}" /></p>    	
